...
class _CameraPermissionPageState extends State<CameraPermissionPage> {
  ...
  Future<void> _pickImageFromGallery() async {
    /*
    * Xfile 은 flutter 에서 주로 파일 선택기나 이미지 파일을 다룰 때 사용하는 파일 처리 클래스이다.
    ...
    return Center(
      child: Column(
        ...
        children: [
          _image == null
              ? Text("사진을 선택하세요")
              : Image.file(_image!), // 변수! 는 null 아니라고 붙여놓는 것이다.
          SizedBox(height: 20),
          // 갤러리에서 선택하는 버튼
          ElevatedButton(
              onPressed: _pickImageFromGallery, child: Text("갤러리에서 사진 선택")),
          ElevatedButton(
              onPressed: _pickImageFromCamera, child: Text("카메라에서 사진 찍기")),
        ],
      ),
    );
  }
