...
class _ContactsPageState extends State<ContactsPage> {
  ...
  // async await 비동기 함수를 사용하려면 반환타입을 Future 로 해줘야 한다. Promise 와 비슷
  ...
  Future<void> _addContact(String name, String phone) async {
    ...
    if (await FlutterContacts.requestPermission()) {
      final newContact = Contact()
        ..name.first = name // .. 종속연산자로 Contact 안의 내용을 불러와서 넣어줌
        ..phones = [
          Phone(phone)
        ]; // phones 가 List 여서 Phone 객체 만들어서 넣어줌 / Phone 객체는 String number 를 매개변수로 받음
      await newContact.insert();

      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text("새 연락처가 저장되었습니다.")));
      _getContacts(); // 연락처 불러오기
    } else {
      ScaffoldMessenger.of(context)
